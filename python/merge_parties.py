...
def merge_parties(cur, keep: dict, merge: dict) -> bool:
    """
    Unisce `merge` in `keep` (dict come restituiti da fetch_party). Scrive
    sempre nel database (nessuna modalità dry-run qui: quella resta solo
    nella CLI). Restituisce True se il partito `merge` è stato eliminato,
    False se restano righe in conflitto da risolvere a mano.
    """
    ...
    try:
        with conn.cursor() as cur:
            ...
            if args.dry_run:
                ...
                logging.info(
                    f"results: {len(merge_result_years - keep_result_years)} anni spostabili, "
                    f"{len(merge_result_years & keep_result_years)} in conflitto"
                )
                logging.info(
                    f"party_codes: {len(merge_code_years - keep_code_years)} anni spostabili, "
                    f"{len(merge_code_years & keep_code_years)} in conflitto"
                )
                logging.info(
                    f"regional_results: {len(merge_regional_keys - keep_regional_keys)} righe spostabili, "
                    f"{len(merge_regional_keys & keep_regional_keys)} in conflitto"
                )
                logging.info("[DRY-RUN] Nessuna modifica scritta.")
                return

            completed = merge_parties(cur, keep, merge)
            conn.commit()
            if completed:
                logging.info(
