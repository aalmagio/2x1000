...
from extract import (
    ...
    parse_amount, parse_int, read_table, sha256_file,
)
...
def parse_results_table(header: "list[str]", rows: "list[list[str]]") -> "list[dict]":
    ...
    """
    header, rows = locate_header(header, rows, PARTY_ALIASES)
...
def main():
    ...
    # Un anno saltato perché non ancora configurato in config.yaml non è un
    # errore della pipeline: esce con successo a meno che non ci sia stato un
    # vero errore di download/parsing.
